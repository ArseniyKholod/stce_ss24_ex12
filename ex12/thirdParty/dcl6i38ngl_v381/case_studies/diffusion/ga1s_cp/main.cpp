...
template <typename TYPE> void EulerSteps(const size_t m, const vector<TYPE>& A, vector<TYPE>& T) {
  size_t n = T.size();
...
template <typename TYPE> inline void sim(const vector<TYPE>& c, const size_t m, vector<TYPE>& T, const size_t cs) {
  size_t n = T.size();
...
int main(int argc, char* argv[]) {
  ...
  f(ca, m, Ta, O, va, cs);
  cerr << "record=" << dco::size_of(tape) << "B" << endl;
  derivative(va) = 1.0;
